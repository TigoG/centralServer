...
export default function WeatherCard({ station = {}, onFocus = () => {} }) {
  ...
  const temp = typeof tempVal === 'number' && tempVal !== -1 ? `${tempVal.toFixed(1)} °C` : '—';
  ...
  return (
    <article className="weather-card" role="article" aria-labelledby={`wc-${id}-title`}>
      <div className="weather-card__left">
        <div className="weather-card__icon" aria-hidden="true">{icon}</div>
      </div>

      <div className="weather-card__body">
        <h3 id={`wc-${id}-title`} className="weather-card__city">{name}</h3>
        <div className="weather-card__date">{new Date().toLocaleDateString()}</div>
      ...
      <div className="weather-card__right">
        ...
        <button type="button" className="weather-card__btn" onClick={onFocus} aria-label={`Focus ${name} on map`}>Focus</button>
      </div>
    </article>
  );
}