    ...
    <div className="app">
      ...
      <div className="controls-area">
        <div className="controls-row">
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', width: '90%', margin: '0 auto' }}>
            <button
              className="btn btn-primary"
              style={{ flex: 1, padding: '12px 36px', height: 56, fontSize: 20, borderRadius: 12, marginTop: 8 }}
              onClick={() => {
                setShowSearch((v) => !v);
                setShowAddForm(false);
                setSearchError(null);
            ...
            <button
              ...
              style={{ flex: 1, padding: '12px 36px', height: 56, fontSize: 20, borderRadius: 12, marginTop: 8, marginLeft: 8 }}
              onClick={() => {
                setShowAddForm((v) => !v);
                setShowSearch(false);
                setSearchError(null);
        ...
        <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
          <div
            ...
            style={{
              ...
              margin: '12px 10%',
              overflow: 'hidden',
              transition: 'max-height 360ms ease, padding 300ms ease, box-shadow 300ms ease',
              maxHeight: showSearch || showAddForm ? 520 : 0,
              padding: showSearch || showAddForm ? '16px' : '0 16px',
              boxSizing: 'border-box',
              background: 'white',
              borderRadius: 8,
              boxShadow: showSearch || showAddForm ? '0 6px 18px rgba(15,23,42,0.06)' : 'none',
            }}
          >
            <div
              style={{
            ...
            >
              ...
              {showAddForm && (
                <div className="add-form" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <input className="controls-input" type="text" placeholder="ID" value={addForm.id} onChange={(e) => setAddForm((f) => ({ ...f, id: e.target.value }))} />
                  <input className="controls-input" type="text" placeholder="Name" value={addForm.name} onChange={(e) => setAddForm((f) => ({ ...f, name: e.target.value }))} />
                  <div className="coord-row">
                    <input className="controls-input" type="number" step="any" placeholder="Lat" value={addForm.lat} onChange={(e) => setAddForm((f) => ({ ...f, lat: e.target.value }))} />
                    <input className="controls-input" type="number" step="any" placeholder="Lon" value={addForm.lon} onChange={(e) => setAddForm((f) => ({ ...f, lon: e.target.value }))} />
                  </div>
                  <select className="controls-input" value={addForm.location} onChange={(e) => setAddForm((f) => ({ ...f, location: e.target.value }))}>
                    <option value="0">Binnen</option>
                    <option value="1">Buiten</option>
                  </select>

                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                    <button className="btn btn-secondary" onClick={() => { setShowAddForm(false); setSearchError(null); }}>
                      Cancel
                    </button>
              ...
              {searchError && <div className="form-error" style={{ marginTop: 8 }}>{searchError}</div>}
            </div>
          </div>
        </div>
      </div>
